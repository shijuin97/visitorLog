<template>
  <master-layout>
    <div class="card shadow mb-4">
      ...
      >
        <h6 class="m-0 font-weight-bold text-primary">Edit a Unit</h6>
      </div>
      ...
      <div class="card-footer" align="center">
        ...
        <a class="btn btn-primary mt-2" @click="editData">Submit</a>
      </div>
    </div>
  </master-layout>
</template>
...
export default {
  ...
  created() {
    ...
    this.id = this.$devUtilities.getValueFormUrlParams("id");
    //redirect to package index if no id is assigned in url
    if (!this.id) {
  ...
  methods: {
    getUnitById: function (id) {
      axios
        .get("/api/getUnitById/" + id)
        .then((response) => {
          this.block = response.data.block;
          this.unit_number = response.data.unit_number;
          this.occupant_name = response.data.occupant_name;
